...
#include <SPI.h>
#include <Wire.h>
#include <WiFiMulti.h>  // From v1.1.1
WiFiMulti wifiMulti;
...
const int lrPin = 33;
debounceButton lrButton(lrPin);

const int anglePin = 35;
debounceButton angleButton(anglePin);
...
void setup()
{
  ...
  angleButton.init(true);

  //speedButton.init(true);

  pinMode(32, INPUT_PULLUP);
  /*attachInterrupt(GPIOPin, ISR, Mode); Mode: HIGH	Triggers the interrupt whenever the pin is HIGH*/
  attachInterrupt(32, ai0, HIGH);
...
void loop()
{
 ...
 //zeroButton
  ...
  { 
    Serial.printf("zeroButton was Klicked\n");
  ...
  { 
    lastWorkingTime=frameTime;
    Serial.printf("zeroButton isPressed\n");
    mb.state = 2;
  ...
  {
    lastWorkingTime=frameTime;
    Serial.printf("zeroButton was Double Klicked\n");
...
//angleButton
 if(angleButton.wasKlicked())
  ...
  mb.zaehler = 0;
  if(speedButton.wasKlicked())
  { 
    lastWorkingTime=frameTime;
    Serial.printf("speedButton was Klicked\n");
    mb.zaehler = 1;
  }
  
  if(speedButton.wasDoubleKlicked())
  {
    lastWorkingTime=frameTime;
    Serial.printf("speedButton was Double Klicked\n");
    mb.zaehler = 2;
  } */
